...
class DependencyChecksTests(unittest.TestCase):
    ...
    def test_check_optional_tool_accepts_tsmuxer_aliases(self):
        with (
            mock.patch.object(
                dependency_checks.shutil,
                "which",
                side_effect=lambda name: "/tmp/tsMuxeR" if name == "tsMuxeR" else None,
            ),
            mock.patch.object(
                dependency_checks,
                "capture_command",
                return_value=subprocess.CompletedProcess(
                    ["/tmp/tsMuxeR"], 0, stdout="tsMuxeR. Version 2.6.16\nusage: ...\n"
                ),
            ),
        ):
    ...
    def test_which_tool_prefers_local_bundle_when_requested(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch.object(
                dependency_checks.shutil, "which", return_value="/usr/bin/tsMuxer"
            ):
    ...
    def test_missing_optional_tool_includes_remediation(self):
        with (
            mock.patch.object(dependency_checks.shutil, "which", return_value=None),
            mock.patch.object(dependency_checks.platform, "system", return_value="Darwin"),
        ):
    ...
    def test_requests_available_reflects_importlib_spec(self):
        with mock.patch.object(
            dependency_checks.importlib.util, "find_spec", return_value=object()
        ):
            ...
            with self.subTest(name=name):
                with (
                    mock.patch.object(
                        dependency_checks.shutil,
                        "which",
                        side_effect=lambda candidate, *, _name=name, _path=path: (
                            _path if candidate == _name else None
                        ),
                    ),
                    mock.patch.object(
                        dependency_checks,
    ...
    def test_check_udf_iso_creator_accepts_each_supported_candidate(self):
        ...
        for tool_name, expected_cmd in cases.items():
            with self.subTest(tool_name=tool_name):
                ...
                with (
                    mock.patch.object(dependency_checks, "which_tool", side_effect=fake_which),
                    mock.patch.object(
                        dependency_checks,
                        "capture_command",
                        return_value=subprocess.CompletedProcess(
                            expected_cmd + ["-help"], 0, stdout="usage: tool with -udf support\n"
                        ),
                    ),
                ):
    ...
    def test_check_udf_iso_creator_reports_non_udf_candidates(self):
        with (
            mock.patch.object(
                dependency_checks,
                "which_tool",
                side_effect=lambda name, root=None, prefer_local=False: (
                    Path("/usr/bin/mkisofs") if name == "mkisofs" else None
                ),
            ),
            mock.patch.object(
                dependency_checks,
                "capture_command",
                return_value=subprocess.CompletedProcess(
                    ["/usr/bin/mkisofs", "-help"],
                    0,
                    stdout="usage without universal-disk-format support\n",
                ),
            ),
        ):
            cmd, detail = dependency_checks.check_udf_iso_creator()
