...
def safe_model():
    return {
        ...
        "playlists": [
            {"playlist_id": "00001", "video_file": "Movie.mkv", "title_number": 1, "kind": "title"}
        ],
    }
...
class MenuBackendCompatibilityTests(unittest.TestCase):
    ...
    def test_bdj_only_feature_forces_auto_to_bdj(self):
        ...
        self.assertTrue(
            any(
                row["feature"] == "motion_or_windowed_menu_video"
    ...
    def test_hdmv_report_reflects_parsed_action_types(self):
        ...
        self.assertTrue(
            any(
                row["feature"] == "timed_or_chapter_video_actions"
                for row in report["bdj_required_features"]
            )
        )
        self.assertTrue(
            any(
                row["feature"] == "bdj_runtime_builtin_actions"
                for row in report["bdj_required_features"]
            )
        )
        self.assertTrue(
            any(row["feature"] == "safe_builtin_actions" for row in report["safe_features"])
        )
    ...
    def test_compatibility_report_files_are_written(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = write_compatibility_report(
                menu_dir, bdj_only_model(), requested_backend="auto", selected_backend="bdj"
            )
            self.assertFalse(report["hdmv_safe"])
            self.assertIn(
                "motion_or_windowed_menu_video", (menu_dir / "menu-compatibility.md").read_text()
            )
            self.assertIn(
                "HDMV-Lite compiler status: `ir_only_first_milestone`",
                (menu_dir / "menu-compatibility.md").read_text(),
            )
    ...
    def test_hdmv_ig_plan_exports_pages_buttons_and_background_objects(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ig_plan["pages"][0]["buttons"][0]["visual_state_refs"]["normal"], "slide1:bg"
            )
            ...
            "button_table": [
                {
                    ...
                    "normal_object_index": 5,
                    "selected_object_index": None,
                    "activated_object_index": None,
                    "action": {},
                }
            ],
            "bog_table": [
    ...
    def test_hdmv_ig_assembly_reports_button_state_dimension_mismatch(self):
        tables = {
            ...
            "object_table": [
                ...
                {
                    "object_index": 1,
                    "id": "slide1:Play:selected",
                    "kind": "button_state_bitmap",
                    "menu_id": "slide1",
                    "button_id": "Play",
                    "state": "selected",
                    "file": "assets/slide1_Play_selected.png",
                    "width": 199,
                    "height": 60,
                },
                {
                    "object_index": 2,
                    "id": "slide1:Play:activated",
                    "kind": "button_state_bitmap",
                    "menu_id": "slide1",
                    "button_id": "Play",
                    "state": "activated",
                    "file": "assets/slide1_Play_activated.png",
                    "width": 200,
                    "height": 59,
                },
            ],
            ...
            "button_table": [
                {
                    ...
                    "select_value": 1,
                    "label": "Play",
                    "hitbox_px": {"x": 10, "y": 20, "w": 200, "h": 60},
                    "neighbors": {},
                    "normal_object_index": 0,
                    "selected_object_index": 1,
                    "activated_object_index": 2,
        ...
        mismatches = [
            row
            for row in assembly["validation"]["errors"]
            if row["type"] == "button_state_dimension_mismatch"
        ]
    ...
    def test_hdmv_ig_assembly_reports_background_and_state_identity_mismatches(self):
        tables = {
            ...
            "object_table": [
                ...
                {
                    "object_index": 1,
                    "id": "slide2:Wrong:selected",
                    "kind": "background_bitmap",
                    "menu_id": "slide2",
                    "button_id": "Wrong",
                    "state": "activated",
                    "file": "assets/slide2_Wrong.png",
                    "width": 200,
                    "height": 60,
                },
                {
    ...
    def test_hdmv_ig_binary_scaffold_packs_deterministic_sections(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [section["name"] for section in binary["sections"]],
                ["header", "pages", "buttons", "bogs", "actions"],
            )
            self.assertEqual(binary["sections"][0]["hex"][:8], "49475343")
            page_section = next(
                section for section in binary["sections"] if section["name"] == "pages"
            )
            button_section = next(
                section for section in binary["sections"] if section["name"] == "buttons"
            )
    ...
    def test_hdmv_movieobject_plan_emits_sample_backed_jump_title_commands(self):
                ...
                "menus": [
                    {
                        ...
                        "buttons": [
                            ...
                            },
                            {
                                "id": "Extras",
                                "label": "Extras",
                                "action": {"type": "go_to_menu", "target_menu": "slide2"},
                            },
                        ],
                    },
                    {
                        ...
                        "buttons": [
                            {
                                "id": "Return",
                                "label": "Main Menu",
                                "action": {"type": "return_main_menu", "target_menu": "slide1"},
                            },
                        ],
                    },
                ],
        ...
        self.assertTrue(
            any(
                obj["kind"] == "menu_page" and obj["menu_id"] == "slide1" for obj in plan["objects"]
            )
        )
        self.assertTrue(
            any(
                route["op"] == "SetButtonPage" and route["target_menu"] == "slide2"
                for route in plan["routes"]
            )
        )

    def test_hdmv_movieobject_command_compiler_marks_fallbacks_without_inventing_unknown_opcodes(
        self,
    ):
        plan = compile_hdmv_movieobject_plan(
            {
    ...
    def test_hdmv_validation_report_describes_available_oracle_commands(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_hdmv_validation_report(
    ...
    def test_hdmv_backend_accepts_safe_menu_and_writes_java_free_package(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ig_container["schema_version"], "auto-bluray-hdmv-ig-packet-container-v1"
            )
            self.assertEqual(
                movieobject_plan["schema_version"], "auto-bluray-hdmv-movieobject-plan-v1"
            )
            self.assertEqual(movieobject_plan["compiler_status"], "compiled_with_fallbacks")
            self.assertEqual(
                validation_report["schema_version"], "auto-bluray-hdmv-validation-report-v1"
            )
            self.assertEqual(validation_run["schema_version"], "auto-bluray-hdmv-validation-run-v1")
            available_validation_commands = sum(
                1 for cmd in validation_report["commands"] if cmd["available"]
            )
            ...
            self.assertEqual(
                ...
                len(validation_report["commands"]) - available_validation_commands,
            )
            self.assertEqual(
                validation_run["passed_count"] + validation_run["failed_count"],
                validation_run["executed_count"],
            )
            self.assertEqual(data["titles"][0]["playlist_id"], "00001")
            self.assertEqual(
                data["menus"][0]["buttons"][0]["state_assets"]["selected"],
                "assets/slide1_Play_selected.png",
            )
            self.assertEqual(
                ig_plan["pages"][0]["buttons"][0]["visual_state_refs"]["selected"],
                "slide1:Play:selected",
            )
            self.assertTrue(any(obj["id"] == "slide1:Play:selected" for obj in ig_plan["objects"]))
            self.assertTrue(
                any(row["id"] == "slide1:Play:selected" for row in ig_tables["object_table"])
            )
            ...
            self.assertTrue(
                any(cmd["tool"] == "movieobject_jar" for cmd in validation_report["commands"])
            )
            ...
            self.assertTrue(
                any(
                    cmd["status"] == "fallback"
                    for obj in movieobject_plan["objects"]
                    for cmd in obj.get("compiled_commands", [])
                )
            )
    ...
    def test_bdj_backend_still_installs_same_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch("menu_backends.run") as mocked_run,
                mock.patch("menu_backends.sign_pptx_menu_jar") as mocked_sign,
            ):
