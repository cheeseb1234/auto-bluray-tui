...
class InstallReportingTests(unittest.TestCase):
    def test_linux_check_only_uses_shared_required_probe_messages(self):
        ...
        def fake_check_tool(name):
            if name == "java":
                raise RuntimeError(
                    "Required dependency 'java' was not found in PATH. Install OpenJDK 8+ and make sure `java` is on PATH."
                )
            return Path(f"/tmp/{name}"), f"{name} ok"

        fake_deps.check_tool.side_effect = fake_check_tool
        fake_deps.check_optional_tool.side_effect = AssertionError(
            "Linux check-only should not probe optional tools"
        )

        stdout = StringIO()
        stderr = StringIO()
        with (
            mock.patch.object(install, "_dependency_checks", return_value=fake_deps),
    ...
    def test_macos_check_only_reports_optional_tsmuxer_probe_message(self):
        ...
        fake_deps.check_optional_tool.side_effect = lambda name: (
            None,
            "not found — install with: Install tsMuxer and ensure tsMuxer, tsMuxeR, or tsmuxer is on PATH",
        )
        ...
        self.assertIn(
            "- tsMuxer: MISSING — not found — install with: Install tsMuxer and ensure tsMuxer, tsMuxeR, or tsmuxer is on PATH",
            text,
        )
