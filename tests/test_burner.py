...
class BurnerStrategyTests(unittest.TestCase):
    ...
    def test_linux_burn_uses_wodim_or_cdrecord_and_dev_sr0_default(self):
        ...
        with (
            tempfile.NamedTemporaryFile(suffix=".iso") as iso,
            mock.patch(
                "tools.burner.shutil.which",
                side_effect=lambda name: "/usr/bin/wodim" if name == "wodim" else None,
            ),
            ...
            stream.assert_called_once_with(
                ["/usr/bin/wodim", "-v", "dev=/dev/sr0", "-eject", iso.name]
            )
    ...
    def test_windows_imgburn_prefers_common_path_and_uses_required_cli_flags(self):
        ...
        with (
            ...
            mock.patch.object(
                Path, "is_file", autospec=True, side_effect=lambda self: self == imgburn_path
            ),
    ...
    def test_windows_isoburn_fallback_warns_and_uses_quiet_command(self):
        ...
        with (
            ...
            mock.patch.object(
                ...
                "is_file",
                autospec=True,
                side_effect=lambda self: self.name.lower() == "isoburn.exe",
            ),
            mock.patch.object(WindowsBurnStrategy, "_stream", return_value=True) as stream,
