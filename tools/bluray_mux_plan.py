...
def main():
    ap = argparse.ArgumentParser(
        description="Create Blu-ray mux/ISO authoring plan from PPTX video actions."
    )
    ...
    root = Path(__file__).resolve().parents[1]
    menu_dir = (
        Path(args.menu_dir).resolve()
        if args.menu_dir
        else root / "xlets/grin_samples/Scripts/PptxMenu"
    )
    ...
    for action in actions:
        ...
        full_length = bool(
            encoded_duration and source_duration and encoded_duration >= max(1, source_duration - 2)
        )
        valid = bool(encoded_duration)
        row = {
            **action,
            "encoded_abs": str(encoded),
            "encoded_exists": exists,
            "ffprobe_ok": valid,
            "encoded_duration_seconds": encoded_duration,
            "source_duration_seconds": source_duration,
            "full_length_encode": full_length,
            "clip_id": action["playlist_id"],
            "stream_file": f"{action['playlist_id']}.m2ts",
        }
        rows.append(row)
...
''')
    ...
    for r in rows:
        ...
        lines.append(
            f"- {r['button']} → `{r['video_file']}` → playlist `{r['playlist_id']}` / title `{r['title_number']}` → `{r['encoded_abs']}` ({status})"
        )
    lines += ["", "## Next commands", ""]
    ...
    lines.append("")
    lines.append(
        "2. Install/add muxing tools before final authoring: `tsMuxer` and `xorriso` or `mkisofs`."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "The final ISO assembly uses tsMuxer output for each title and patches playlist/clip IDs to match the PowerPoint button map. Validate the ISO in software before burning physical media."
    )
