...
def detect_burners():
    ...
    for dev in sorted(Path("/dev").glob("sr*")):
        ...
        with contextlib.suppress(Exception):
            vendor = (
                (Path("/sys/block") / name / "device/vendor").read_text(errors="ignore").strip()
            )
        with contextlib.suppress(Exception):
            model = (Path("/sys/block") / name / "device/model").read_text(errors="ignore").strip()
        burners.append(
            {"device": str(dev), "label": " ".join(x for x in (vendor, model) if x).strip() or name}
        )
    return burners
...
def main():
    ap = argparse.ArgumentParser(
        description="Detect Blu-ray burner/media and burn final ISO when safe."
    )
    ap.add_argument("iso")
    ...
    ap.add_argument(
        "--auto",
        action="store_true",
        help="burn automatically only if a detected burner has enough blank/appendable space",
    )
    ...
    status = {
        ...
        "burners": checked,
        "ready_device": ready["device"] if ready else None,
    }
    if args.status_json:
    ...
    if args.auto:
        ...
        if not ready:
            raise SystemExit(
                "No burner with blank/appendable media large enough for ISO was detected; skipping automatic burn."
            )
        burn(xorriso, ready["device"], iso)
        print(
            "Burn complete. Insert another blank disc and use the TUI burn command for additional copies, or exit.",
            flush=True,
        )
        return 0
