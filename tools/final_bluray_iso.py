...
def validate_encoded(
    ...
    *,
    ...
    if source_duration and duration < source_duration - 2:
        raise SystemExit(
            f"Encoded file is not full length: {path} ({duration:.1f}s < {source_duration:.1f}s)"
        )
    if not video or video.get("codec_name") != "h264":
        raise SystemExit(f"Encoded file is not H.264 video: {path}")
    if (
        not audio
        or audio.get("codec_name") != "ac3"
        or str(audio.get("sample_rate") or "") != "48000"
    ):
    ...
    if not allow_oversized and max_bitrate and bit_rate and bit_rate > max_bitrate:
        raise SystemExit(
            f"Encoded file bitrate too high for selected disc target: {path} ({bit_rate / 1_000_000:.1f} Mb/s)"
        )
    return {
        "duration": duration,
        "bit_rate": bit_rate,
...
def matching_subtitles(project: Path, video_file: str, manifest: dict) -> list[dict]:
    ...
    rows.sort(
        key=lambda r: (Path(r["path"]).stem.lower() != video_stem, Path(r["path"]).name.lower())
    )
    return rows
...
def validate_final_disc(disc_root: Path, menu_backend: str):
    ...
    path_hits = [
        str(p.relative_to(disc_root))
        for p in disc_root.rglob("*")
        if any(s in str(p) for s in forbidden)
    ]
    if path_hits:
        raise SystemExit(
            "Legacy sample artifact paths found in final disc: " + ", ".join(path_hits[:20])
        )
    ...
    if menu_backend == "bdj":
        with zipfile.ZipFile(jar_path) as zf:
            names = zf.namelist()
            grin_text = (
                zf.read("pptx-menu.grin").decode("utf-8", errors="ignore")
                if "pptx-menu.grin" in names
                else ""
            )
        jar_hits = [n for n in names if any(s in n for s in forbidden)]
        if jar_hits:
            raise SystemExit(
                "Legacy sample artifacts found in final menu jar: " + ", ".join(jar_hits[:20])
            )
        ...
        if not bg_assets:
            raise SystemExit(
                "Generated PPTX slide background assets are missing from final menu jar"
            )
        ...
        for line in grin_text.splitlines():
            ...
            referenced_assets.update(
                p for p in parts if p.startswith("assets/slide") and p.endswith(".png")
            )
        missing_assets = sorted(referenced_assets - slide_assets)
        if missing_assets:
            raise SystemExit(
                "Generated PPTX assets are missing from final menu jar: "
...
def main():
    ap = argparse.ArgumentParser(
        description="Assemble final Blu-ray BDMV tree and ISO from encoded videos + HD Cook Book menu overlay."
    )
    ...
    ap.add_argument(
        "--volume-id",
        default=None,
        help="disc title / ISO volume label; sanitized to a 32-character volume ID",
    )
    ap.add_argument(
        "--disc-preset",
        choices=["quality", "dvd5", "dvd9", "bd25"],
        default="bd25",
        help="disc size target for final capacity validation",
    )
    ap.add_argument(
        "--allow-oversized",
        action="store_true",
        help="allow outputs above selected disc bitrate/size guardrails",
    )
    ap.add_argument("--no-iso", action="store_true", help="build final BDMV tree only")
    ap.add_argument(
        "--menu-backend",
        choices=MENU_BACKENDS,
        default=DEFAULT_MENU_BACKEND,
        help="menu authoring backend: bdj/GRIN working default, hdmv experimental scaffold, or auto-select; default: bdj",
    )
    args = ap.parse_args()
    ...
    output_root = (
        Path(args.output_root).resolve() if args.output_root else project / "build" / "final-bluray"
    )
    ...
    menu_dir, menu_model, menu_compatibility, selected_menu_backend = convert_pptx_menu(
        root, project, requested_backend=args.menu_backend
    )
    ...
    menu_install = install_menu_backend(
        root, project, menu_dir, menu_model, selected_menu_backend, disc_root, output_root
    )
    ...
    durations = {
        v["file"]: float(v.get("duration_seconds") or 0) for v in manifest.get("videos", [])
    }
    total_duration = sum(durations.values())
    max_bitrate = None
    if args.disc_preset in DISC_CAPACITY_BYTES and total_duration:
        max_bitrate = int(
            (DISC_CAPACITY_BYTES[args.disc_preset] * 8 * 0.88 / total_duration + 500_000) * 1.25
        )
    ...
    for row in rows:
        ...
        info = validate_encoded(
            encoded,
            durations.get(row.get("video_file")),
        ...
        summary.append(
            {
                ...
                **info,
                ...
                "subtitles": [
                    {
                        "file": s["path"].name,
                        "language": s.get("language") or subtitle_language(s["path"]),
                    }
                    for s in subtitles
                ],
            }
    ...
    if not args.no_iso:
        ...
        if capacity and not args.allow_oversized and report["iso_bytes"] > capacity:
            raise SystemExit(
                f"Final ISO is too large for {args.disc_preset}: {report['iso_bytes']} bytes > {capacity} bytes"
            )
        (output_root / "final-report.json").write_text(json.dumps(report, indent=2) + "\n")
