...
def resolve_video_target(
    name: str, videos: dict[str, str], *, exact: bool = False
    ...
    if scored and scored[0][0] >= 0.88:
        ...
        if len(top_hits) == 1 and top_score - second_score >= 0.08:
            return scored[0][2], [
                f'fuzzy label match: "{label}" -> "{scored[0][2]}" ({top_score:.2f})'
            ]
...
def parse_button_action(
    ...
    warnings: list[str] = []
    if not text:
        return {"kind": "builtin", "name": "none", "type": "none"}, [
            "empty action text; treating button as disabled"
        ]
        ...
        if lower.startswith(prefix):
            ...
            if not target:
                return {"kind": "invalid", "raw": text}, [
                    f"{prefix[:-1]} action is missing a slide name"
                ]
            action = {
                "kind": "slide",
                "target": target,
    ...
    if "#" in video_label:
        ...
        if not video_label or not chapter_text:
            return {"kind": "invalid", "raw": text}, [
                f'invalid chapter action "{text}"; expected VideoName#ChapterName'
            ]
    ...
    if "@" in video_label:
        ...
        if not video_label or not timestamp_text:
            return {"kind": "invalid", "raw": text}, [
                f'invalid timestamp action "{text}"; expected VideoName@timestamp'
            ]
    ...
    if not video_file:
        return {"kind": "unresolved", "raw": text, "target_label": video_label}, warnings or [
            f"video target not found: {video_label}"
        ]
