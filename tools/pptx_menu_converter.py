...
def project_videos(project_dir: Path) -> list[Path]:
    return sorted(
        [p for p in project_dir.iterdir() if p.is_file() and p.suffix.lower() in VIDEO_EXTS],
...
def find_project_pptx(project_dir: Path) -> Path | None:
    ...
    if len(pptxs) > 1:
        ...
        raise SystemExit(
            f"Multiple .pptx files found in {project_dir}; keep exactly one menu PPTX. Found: {names}"
        )
    return pptxs[0]
...
def ppt_shape(
    ...
    *,
    ...
    line = (
        ...
        if title
        else '<a:ln w="0"><a:solidFill><a:srgbClr val="3465a4"/></a:solidFill></a:ln>'
    )
...
def ppt_slide_xml(title: str, buttons: list[dict], slide_w: int, slide_h: int) -> str:
    title_w = int(slide_w * 0.68)
    title_h = 520000
    title_x = (slide_w - title_w) // 2
    title_y = 360000
    btn_w = int(slide_w * 0.27)
    btn_h = 685800
    start_y = int(slide_h * 0.34)
    gap_y = 230000
    ...
    for i, b in enumerate(normal):
        ...
        shapes.append(ppt_shape(sid, b["label"], x, y, btn_w, btn_h, b.get("rid")))
        sid += 1
...
def generate_menu_pptx_from_template(
    project_dir: Path, pptx: Path, template: Path = DEFAULT_MENU_TEMPLATE
):
    ...
    with zipfile.ZipFile(template) as zin:
        ...
        with zipfile.ZipFile(tmp, "w", zipfile.ZIP_DEFLATED) as zout:
            for item in zin.infolist():
                if item.filename in skip or re.match(
                    r"ppt/slides/(?:_rels/)?slide\d+\.xml(?:\.rels)?$", item.filename
                ):
                    continue
            ...
            zout.writestr(
                "ppt/slides/slide1.xml", ppt_slide_xml("Main Menu", main_buttons, slide_w, slide_h)
            )
            ...
            for idx, group in enumerate(groups, 2):
                ...
                zout.writestr(
                    ...
                    ppt_slide_xml(f"Videos {idx - 1}", buttons, slide_w, slide_h),
                )
                zout.writestr(
                    f"ppt/slides/_rels/slide{idx}.xml.rels", ppt_slide_rels([("rIdMain", 1)])
                )
            sld_ids = "".join(
                f'<p:sldId id="{255 + i}" r:id="rId{2 + i}"/>' for i in range(1, slide_count + 1)
            )
...
<p:presentation xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><p:sldMasterIdLst><p:sldMasterId id="2147483648" r:id="rId2"/></p:sldMasterIdLst><p:sldIdLst>{sld_ids}</p:sldIdLst><p:sldSz cx="{slide_w}" cy="{slide_h}"/><p:notesSz cx="7772400" cy="10058400"/></p:presentation>'''
            ...
            pres_rels = [
                '<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme" Target="theme/theme1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideMaster" Target="slideMasters/slideMaster1.xml"/>'
            ]
            ...
            pres_rels.append(
                '<Relationship Id="rId999" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/presProps" Target="presProps.xml"/></Relationships>'
            )
...
def find_video_match(label: str, videos: dict[str, str]):
    ...
    relaxed_hits = sorted(
        {name for key, name in videos.items() if relaxed and relaxed_key(key) == relaxed}
    )
    ...
    subset_hits = sorted(
        {
            ...
            if label_words and label_words <= set(relaxed_key(key).split())
        }
    )
    ...
    if (
        scored
        and scored[0][0] >= 0.88
        and (len(scored) == 1 or scored[0][0] - scored[1][0] >= 0.08)
    ):
        return scored[0][
            2
        ], f'fuzzy label match: "{label}" -> "{scored[0][2]}" ({scored[0][0]:.2f})'
    return None, None
        ...
        for btn in slide.get("buttons") or []:
            ...
            if (
                ...
                if target:
                    ...
                    warning = (
                        f'goto target "{action["target_label"]}" does not match any slide title/id'
                    )
                    btn.setdefault("parse_warnings", []).append(warning)
                    model.setdefault("match_warnings", []).append(
                        {
                            "slide": slide.get("id"),
                            "label": btn.get("raw_text") or btn.get("label"),
                            "message": warning,
                        }
                    )
...
def repair_conflicting_slide_links(model: dict) -> None:
    ...
    for slide in model.get("slides") or []:
        ...
        targets = sorted(
            {
                (btn.get("link_action") or {}).get("target")
                for btn in conflicted
                if (btn.get("link_action") or {}).get("target")
            }
        )
        ...
        original_target_label = (candidate.get("action") or {}).get(
            "target_label"
        ) or candidate.get("label")
        stale_warning = f"video target not found: {original_target_label}"
        candidate["parse_warnings"] = [
            w for w in candidate.get("parse_warnings", []) if w != stale_warning
        ]
        model["match_warnings"] = [
            item
            for item in (model.get("match_warnings") or [])
            if not (
                item.get("slide") == slide.get("id")
                and (
                    item.get("label") == candidate.get("raw_text")
                    or item.get("label") == candidate.get("label")
                )
            ...
            for sp in root.findall(".//p:sp", NS):
                ...
                for h in sp.findall(".//a:hlinkClick", NS):
                    ...
                    if "firstslide" in action:
                        ...
                        if m:
                            link = {"kind": "slide", "target": f"slide{m.group(1)}"}
                ...
                if (
                    link
                    and not has_explicit_action
                    and parsed_action.get("kind") in ("unresolved", "invalid")
                ):
                    parsed_action = None
                    parse_warnings = []
                for warning in parse_warnings:
                    match_warnings.append(
                        {"slide": f"slide{idx}", "label": text, "message": warning}
                    )
                video_name = (
                    parsed_action.get("target")
                    if parsed_action and parsed_action.get("kind") == "video"
                    else None
                )
                if is_loop_placeholder(action_text, video_name, link, rect, (src_w, src_h)):
                    loop_videos.append(
                        {
                            "id": unique_id(display_text, used_ids),
                            "label": display_text,
                            "raw_text": text,
                    ...
                    continue
                parsed_is_button = parsed_action and parsed_action.get("kind") not in (
                    "unresolved",
                    "invalid",
                )
                ...
                action = (
                    parsed_action
                    if parsed_is_button
                    else (link or (parsed_action if button_shape and parsed_action else None))
                )
                if action:
                    buttons.append(
                        {
...
def export_slide_pngs(pptx: Path, out_assets: Path, target=(1920, 1080)):
    ...
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        subprocess.run(
            ["libreoffice", "--headless", "--convert-to", "pdf", "--outdir", str(td), str(pptx)],
            check=True,
...
def generate_loop_source_videos(
    model, project_dir: Path, assets: Path, target=(1920, 1080), seconds=30
):
    ...
    for slide in model["slides"]:
        ...
        for i, loop in enumerate(loops, 1):
            r = loop["rect_px"]
            filters.append(
                f"[{i}:v]setpts=PTS-STARTPTS,scale={r['w']}:{r['h']}:force_original_aspect_ratio=increase,crop={r['w']}:{r['h']},format=yuv420p[v{i}]"
            )
            out_label = f"vout{i}"
    ...
    if loop_sources:
        loop_dir.mkdir(parents=True, exist_ok=True)
        (loop_dir / "source-videos.json").write_text(
            json.dumps(sorted(loop_sources), indent=2) + "\n"
        )
    ...
    for slide in model["slides"]:
        ...
        for btn in slide["buttons"]:
            ...
            crop_box = (
                max(0, x - pad),
                max(0, y - pad),
                min(target[0], x + w + pad),
                min(target[1], y + h + pad),
            )
            if any(
                not (x + w <= lx1 or x >= lx2 or y + h <= ly1 or y >= ly2)
                for lx1, ly1, lx2, ly2 in loop_rects
            ):
            ...
            for state, outline in [
                ("selected", (255, 255, 255, 245)),
                ("activated", (64, 255, 170, 255)),
            ]:
                ...
                crop = (
                    original_bg.crop(crop_box) if btn.get("normal_overlay") else bg.crop(crop_box)
                )
                im.paste(crop, (crop_box[0] - (x - pad), crop_box[1] - (y - pad)))
                d = ImageDraw.Draw(im)
                d.rounded_rectangle(
                    (2, 2, w + pad * 2 - 2, h + pad * 2 - 2), radius=22, outline=outline, width=5
                )
...
def button_grid(buttons):
    ...
    rows = []
    for btn in sorted(
        buttons, key=lambda b: (b["rect_px"]["y"] + b["rect_px"]["h"] / 2, b["rect_px"]["x"])
    ):
...
def visual_rc_grid(buttons):
    ...
    for row_index, row in enumerate(rows):
        ...
        for btn in row:
            ...
            col = min(
                (i for i in range(width) if i not in used), key=lambda i: abs(cx - centers[i])
            )
            cells[col] = btn["id"]
            used.add(col)
...
def add_neutral_menu_metadata(model, assets: Path, target=(1920, 1080)):
    ...
    for slide in model.get("slides", []):
        ...
        for index, btn in enumerate(slide.get("buttons", []), 1):
            ...
            btn["focus_index"] = (
                focus_order.index(btn["id"]) + 1 if btn["id"] in focus_order else index
            )
            action = btn.get("action") or {}
            action.setdefault(
                "type",
                ...
                if action.get("kind") == "slide"
                else action.get("kind", "unknown"),
            )
            if action.get("kind") == "video":
    ...
    if analyze_menu_compatibility:
        ...
        model["feature_requirements"] = (
            report["safe_features"]
            + report["bdj_required_features"]
...
def generate_show(model, out: Path):
    ...
    lines += [
        ...
        f"        F:{first}.BG",
        "    } setup_done {",
        f"        activate_segment S:{first}.Enter ;",
        "    }",
        ";",
        "",
    ]
    ...
    for slide in model["slides"]:
        ...
        for btn in slide["buttons"]:
            ...
            if btn.get("normal_overlay"):
                lines.append(
                    f'feature fixed_image F:{sid}.{bid}.Normal {x} {y} "assets/{sid}_{bid}_normal.png" ;'
                )
            lines.append(
                f'feature fixed_image F:{sid}.{bid}.Selected {x} {y} "assets/{sid}_{bid}_selected.png" ;'
            )
            lines.append(
                f'feature fixed_image F:{sid}.{bid}.Activated {x} {y} "assets/{sid}_{bid}_activated.png" ;'
            )
        lines.append("")
    for slide in model["slides"]:
        sid = slide["id"]
        ...
        for btn in buttons:
            ...
            if act["kind"] == "slide":
                ...
                lines.append(
                    f"        {bid} {bid}_activated {{ activate_segment S:{target}.Enter ; }}"
                )
            elif act["kind"] == "video":
                ...
                lines.append(
                    f'        {bid} {bid}_activated {{ activate_segment S:VideoPlayback ; sync_display ; java_command [[ playVideo("{video}", "{playlist}", "S:{sid}.Enter", {start}); ]] }}'
                )
            elif act["kind"] == "builtin":
                name = act.get("name") or act.get("type")
                if name in ("main", "top_menu"):
                    lines.append(
                        f"        {bid} {bid}_activated {{ activate_segment S:{first}.Enter ; }}"
                    )
                    ...
                    lines.append(
                        f"        {bid} {bid}_activated {{ java_command [[ stopMenuLoop(); ]] }}"
                    )
            else:
        ...
        for btn in buttons:
            r = btn["rect_px"]
            lines.append(
                f"        {btn['id']} ( {r['x']} {r['y']} {r['x'] + r['w']} {r['y'] + r['h']} )"
            )
        lines += ["    }", ";", ""]
...
def main():
    if len(sys.argv) != 3:
        print("usage: pptx_menu_converter.py PROJECT_DIR OUTPUT_DIR", file=sys.stderr)
        sys.exit(2)
    ...
    (out / "video-actions.json").write_text(
        json.dumps((model.get("video_actions", []) + model.get("loop_actions", [])), indent=2)
        + "\n"
    )
    (out / "loop-actions.json").write_text(
        json.dumps(model.get("loop_actions", []), indent=2) + "\n"
    )
    print(f"Generated {out}")
    print(
        f"Slides: {len(model['slides'])}; buttons: {sum(len(s['buttons']) for s in model['slides'])}"
    )
