...
def clean_query(stem: str):
    ...
    text = re.sub(
        r"\b(2160p|1080p|720p|4k|uhd|bluray|blu ray|remux|x264|x265|h264|h265|hevc|aac|ac3|dts|truehd)\b",
        " ",
        text,
        flags=re.I,
    )
...
def search(
    ...
    r = session.get(
        base_url + "/subtitles", headers=headers(api_key, token), params=params, timeout=30
    )
    r.raise_for_status()
...
def download_subtitle(
    ...
    r = session.post(
        base_url + "/download",
        headers={**headers(api_key, token), "Content-Type": "application/json"},
...
def main():
    ap = argparse.ArgumentParser(
        description="Fetch missing sidecar subtitles from OpenSubtitles.com for a Blu-ray project."
    )
    ap.add_argument("project_dir")
    ap.add_argument(
        "--language",
        default=os.environ.get("OPENSUBTITLES_LANGUAGE", "en"),
        help="OpenSubtitles language code, default en",
    )
    ap.add_argument(
        "--force", action="store_true", help="download even when a matching sidecar already exists"
    )
    ...
    videos = sorted(
        [p for p in project.iterdir() if p.suffix.lower() in VIDEO_EXTS],
    ...
    if not api_key:
        report.update(
            {
                "status": "skipped",
                "message": "OPENSUBTITLES_API_KEY is not set; skipping subtitle lookup",
            }
        )
    ...
    if not username or not password:
        report.update(
            {
                "status": "skipped",
                "message": "OPENSUBTITLES_USERNAME/PASSWORD are not set; downloads require an OpenSubtitles login",
            }
        )
        print(json.dumps(report, indent=2))
        return 0
