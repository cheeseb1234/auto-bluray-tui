...
def video_files(project: Path):
    try:
        return sorted(
            [p for p in project.iterdir() if p.suffix.lower() in SUPPORTED_VIDEO_EXTS],
            key=lambda p: p.name.lower(),
        )
    except Exception:
        return []
        ...
        files.extend(
            sorted(
                [
                    p
                    for p in project.iterdir()
                    if p.suffix.lower() in (".srt", ".sup", ".ass", ".ssa")
                ],
                key=lambda p: p.name.lower(),
            )
        )
...
def project_diagnostics(project: Path, root: Path, rows=None, meta=None, cfg=None):
    ...
    if not tools.get("tsMuxer"):
        add(
            "warning",
            "tsMuxer is not available yet; final authoring and ISO creation will fail later.",
            remediation_hint("tsMuxer"),
        )
    if not tools.get("xorriso"):
        add(
            "warning",
            "xorriso is not available yet; direct ISO burning from the TUI will fail later.",
            remediation_hint("xorriso"),
        )
    ...
    if not menu:
        if vids and pptx_menu_converter:
            add(
                "info",
                "Missing .pptx menu; autopilot will generate and open an editable menu template before continuing.",
                "Save/exit PowerPoint or LibreOffice after edits, then press any key in the TUI.",
            )
        else:
            add(
                "error",
                "Missing .pptx menu in the project folder.",
                "Add/export one PowerPoint menu, or add videos so a template can be generated.",
            )
    if not vids:
        add(
            "error",
            "No supported video files found beside the menu PPTX.",
            "Use .mkv, .mp4, .m2ts, or .mov files in the project folder.",
        )

    normalized = {}
    for p in vids:
        k = (
            pptx_menu_converter.match_key(p.stem)
            if pptx_menu_converter
            else re.sub(r"[^a-z0-9]+", " ", p.stem.lower()).strip()
        )
    ...
    if dupes:
        add(
            "warning",
            "Some video filenames normalize to the same title, which can make button matching ambiguous.",
            "Rename one of: " + "; ".join(", ".join(d) for d in dupes[:3]),
        )
    ...
    for video in vids:
        ...
        has_sidecar = any(
            p.suffix.lower() in subtitle_exts and p.stem.lower().startswith(stem)
            for p in project.iterdir()
        )
        if not has_sidecar:
    ...
    if videos_without_sidecars and not os.environ.get("OPENSUBTITLES_API_KEY"):
        add(
            "info",
            "No sidecar subtitles found for some videos; OpenSubtitles lookup is disabled because OPENSUBTITLES_API_KEY is not set.",
            "Set OPENSUBTITLES_API_KEY plus OPENSUBTITLES_USERNAME/PASSWORD to auto-download .srt files.",
        )
    elif videos_without_sidecars:
        add(
            "info",
            "Some videos have no sidecar subtitles; autopilot will search OpenSubtitles before analyzing media.",
            ", ".join(videos_without_sidecars[:6]),
        )

    if os.environ.get("OPENSUBTITLES_API_KEY") and not requests_available():
        add(
            "error",
            "OpenSubtitles fetching is enabled, but the Python requests package is not available to helper scripts.",
            "Launch the packaged app so helper scripts reuse its Python runtime, or install requests with: python3 -m pip install --user requests",
        )
    ...
    if menu and menu.exists() and vids and pptx_menu_converter:
        try:
            ...
            if not slides:
                add(
                    "error",
                    f"{menu.name} contains no slides.",
                    "Add at least one slide with video buttons.",
                )
            if not buttons:
                add(
                    "error",
                    "No clickable PPTX menu buttons were detected.",
                    "Use button text that matches a video name or add slide hyperlinks.",
                )
            if not actions:
                add(
                    "error",
                    "No PPTX buttons match any video files.",
                    "Rename button text to match video filenames; fuzzy/space/punctuation fixes are automatic when safe.",
                )
            loop_targets = {
                loop.get("video_file") for slide in slides for loop in slide.get("loop_videos", [])
            }
            ...
            if missing_targets:
                add(
                    "error",
                    "PPTX menu points at video files that are missing.",
                    "Missing: " + ", ".join(missing_targets[:6]),
                )
            unused = sorted({p.name for p in vids} - {t for t in targets if t})
            if unused and actions:
                add(
                    "warning",
                    "Some project videos are not reachable from the PPTX menu.",
                    "Unreferenced: " + ", ".join(unused[:6]),
                )
            if analyze_menu_compatibility:
                ...
                if backend == "hdmv":
                    if compat.get("hdmv_safe"):
                        add(
                            "warning",
                            "menu_backend=hdmv is selected and this menu looks HDMV-safe, but HDMV-Lite compilation is scaffold-only in this release.",
                            "Use menu_backend=bdj for a working disc until HDMV-Lite compilation is implemented.",
                        )
                    else:
                        add(
                            "warning",
                            "menu_backend=hdmv is selected, but this menu requires BD-J features.",
                            "Use menu_backend=auto/bdj or remove BD-J-only features; see generated menu-compatibility.md after conversion.",
                        )
                elif backend == "auto" and compat.get("hdmv_safe"):
                    add(
                        "info",
                        "menu_backend=auto will stay on BD-J because HDMV-Lite compilation is scaffold-only in this release.",
                        "Auto must not choose HDMV until HDMV compiler_status is functional.",
                    )
            for warn in model.get("match_warnings") or []:
                add(
                    "info",
                    warn.get("message", "Auto-corrected a menu/video filename match."),
                    "Review generated video-actions.json if this was unexpected.",
                )
        except Exception as e:
            add(
                "error",
                f"Could not parse {menu.name} for preflight checks: {e}",
                "Open/re-save the PPTX, then retry.",
            )
    elif menu and menu.exists() and vids and not pptx_menu_converter:
        add(
            "warning",
            "PPTX preflight parser could not be loaded.",
            "Run the converter step to reveal menu issues.",
        )
    ...
    for row in rows:
        ...
        if status == "partial":
            add(
                "error",
                f"{row.get('file')} appears partially encoded.",
                "Re-run encode/autopilot; existing partial outputs will be replaced when needed.",
            )
        elif status == "oversized":
            add(
                "error",
                f"{row.get('file')} is too large for the selected disc target.",
                "Choose a larger disc target or re-encode with current disc setting.",
            )
        elif status == "stale":
            add(
                "warning",
                f"{row.get('file')} has a stale encoder PID.",
                "Press k to clear running state, then rerun encode/autopilot.",
            )
    ...
    if (
        ...
        and not pid_running(workflow.get("pid"))
    ):
        add(
            "warning",
            f"Autopilot state is stale at step {workflow.get('step')}.",
            "The TUI will treat it as stopped; press w to rerun safely.",
        )
    ...
    if iso.exists() and menu and menu.exists() and iso.stat().st_mtime < menu.stat().st_mtime:
        add(
            "warning",
            f"The final ISO is older than {menu.name}.",
            "Rebuild the final ISO before burning.",
        )
    if model and report.exists():
        try:
            ...
            report_titles = {
                t.get("video_file")
                for t in report_data.get("titles", [])
                if t.get("kind", "button") == "button"
            }
            expected_titles = {
                a.get("video_file")
                for a in model.get("video_actions", [])
                if a.get("kind", "button") == "button"
            }
            if expected_titles and report_titles and report_titles != expected_titles:
                add(
                    "warning",
                    "The final report does not match the current PPTX video button actions.",
                    "Rebuild final ISO; playlist map may be stale.",
                )
        except Exception:
            pass
    ...
    try:
        result = subprocess.run(
            [
                ...
                "error",
                "-show_entries",
                "format=duration,bit_rate:stream=codec_type,codec_name,width,height,sample_rate",
                "-of",
                "json",
                str(path),
            ],
...
def tool_status(root: Path):
    return {
        "ffmpeg": str(path)
        if (path := dependency_checks.which_tool("ffmpeg", root=root, prefer_local=True))
        else None,
        "ffprobe": str(path)
        if (path := dependency_checks.which_tool("ffprobe", root=root, prefer_local=True))
        else None,
        "tsMuxer": str(path)
        if (path := dependency_checks.which_tool("tsMuxer", root=root, prefer_local=True))
        else None,
        "xorriso": str(path)
        if (path := dependency_checks.which_tool("xorriso", root=root, prefer_local=True))
        else None,
...
def running_rows(rows):
    return [
        r
        for r in rows
        if r.get("pid") and r.get("status") == "running" and pid_running(r.get("pid"))
    ]
...
def stop_running_work(project: Path, rows):
    ...
    for row in running_rows(rows):
        ...
        state_path = (
            ...
            / "bluray-media"
            / "logs"
            / (row.get("file", "").replace("/", "_") + ".state.json")
        )
        state = read_json(state_path) or {}
        state.update(
            {
                "status": "stopping" if ok else state.get("status", "running"),
    ...
    if (
        workflow.get("pid")
        and workflow.get("status") == "running"
        and pid_running(workflow.get("pid"))
    ):
        ok, msg = stop_pid_group(workflow.get("pid"))
        stopped.append(f"autopilot: {msg}")
        workflow.update(
            {
                "status": "stopping" if ok else workflow.get("status"),
                "updated_at": time.time(),
                "stop_message": msg,
            }
        )
...
def detect_burners():
    ...
    for dev in sorted(Path("/dev").glob("sr*")):
        ...
        with contextlib.suppress(Exception):
            model = (
                Path("/sys/block").joinpath(name, "device/model").read_text(errors="ignore").strip()
            )
        with contextlib.suppress(Exception):
            vendor = (
                Path("/sys/block")
                .joinpath(name, "device/vendor")
...
def media_capacity_bytes(xorriso: str, device: str):
    try:
        ...
        blankish = bool(
            re.search(
                r"Media status\s*:?\s*(is )?(blank|appendable|overwriteable|formatted)",
                text,
                re.IGNORECASE,
            )
        )
...
def start_burn(root: Path, project: Path, device: str):
    ...
    if capacity < iso.stat().st_size:
        raise RuntimeError(
            f"Selected disc is too small or unreadable: {human_size(capacity)} available for {human_size(iso.stat().st_size)} ISO."
        )
    if not blankish:
        raise RuntimeError(
            "Selected burner does not appear to contain blank/appendable/overwriteable media."
        )
    d = burn_dir(project)
    d.mkdir(parents=True, exist_ok=True)
        ...
        log.write(
            (
                "\n=== Burn start {} device={} iso={} ===\n".format(
                    time.strftime("%Y-%m-%d %H:%M:%S"), device, iso
                )
            ).encode()
        )
        proc = subprocess.Popen(
...
def stop_burn(project: Path):
    ...
    if state.get("pid") and state.get("status") == "running" and pid_running(state.get("pid")):
        ...
        state.update(
            {
                "status": "stopping" if ok else state.get("status"),
                "updated_at": time.time(),
                "message": msg,
            }
        )
...
def collect(project: Path, root: Path):
    ...
    for item in manifest.get("videos", []):
        ...
        out_info = (
            {} if raw_status == "running" else (ffprobe_output_info(out) if out.exists() else {})
        )
        ...
        if status == "done" and encoded_duration and duration and encoded_duration < duration - 2:
            status = "partial"
        max_preset_bitrate = estimated_preset_total_bitrate(
            cfg.get("disc_preset"),
            sum(float(v.get("duration_seconds") or 0) for v in manifest.get("videos", [])),
        )
        if (
            status == "done"
            and max_preset_bitrate
            and out_info.get("bit_rate")
            and out_info.get("bit_rate") > max_preset_bitrate
        ):
            status = "oversized"
        if not status:
    ...
    total_done = sum(
        float(r.get("progress_seconds") or r.get("encoded_duration") or 0) for r in rows
    )
    ...
    meta = {
        ...
        "workflow": workflow,
        "burners": burners,
        "selected_burner": selected_burner(project, burners),
        "burn": burn,
        "final_iso": str(iso),
        "final_iso_exists": iso.exists(),
...
def draw_diagnostics(stdscr, y, width, diagnostics, max_rows=5):
    if not diagnostics:
        safe_add(
            ...
            y,
            0,
            "Preflight: ok - naming/menu/navigation/ISO checks passed",
            width - 1,
            color("ok"),
        )
        return y + 1
    ...
    safe_add(
        ...
        y,
        0,
        f"Preflight: {errors} error(s), {warnings} warning(s)  (autocorrect handles safe naming, hitbox, navigation-grid, slide-count, stale-map issues)",
        width - 1,
        color("bad_bold" if errors else "warn_bold"),
    )
    y += 1
    ...
    if len(diagnostics) > max_rows:
        safe_add(
            ...
            y,
            0,
            f"  ... {len(diagnostics) - max_rows} more preflight item(s)",
            width - 1,
            color("dim"),
        )
        y += 1
...
def prompt_disc_title(stdscr, project: Path, cfg: dict):
    ...
    safe_add(
        stdscr, 1, 2, "Auto Blu-ray TUI setup", safe_width, color("accent_bold", curses.A_BOLD)
    )
    ...
    safe_add(
        ...
        4,
        2,
        "This becomes the ISO/UDF volume label; blank uses the project folder name.",
        safe_width,
        color("dim"),
    )
...
def ensure_menu_template_interactive(stdscr, project: Path, root: Path):
    ...
    for i, line in enumerate(lines):
        safe_add(
            ...
            line,
            w - 4,
            color("accent_bold", curses.A_BOLD) if i == 0 else curses.A_NORMAL,
        )
    stdscr.refresh()
    stdscr.getch()
    stdscr.nodelay(True)
    ...
    return (
        True,
        f"Generated {menu.name}; HTML preview will be at {preview}; continuing autopilot after user confirmation.",
    )
...
def draw(stdscr, project: Path, root: Path):
    ...
    if reason:
        ...
        safe_add(
            ...
            1,
            2,
            "Analyzing project/media before showing the dashboard...",
            max(20, width - 4),
            color("info_bold", curses.A_BOLD),
        )
        ...
        safe_add(
            ...
            5,
            2,
            "This gives you the video/subtitle inventory before choosing autopilot or encode-only.",
            max(20, width - 4),
        )
        stdscr.refresh()
        rc, log = run_initial_analyze(root, project)
        message = (
            f"Initial analysis complete; log {log}"
            if rc == 0
            else f"Initial analysis failed rc={rc}; log {log}"
        )
    while True:
        ...
        safe_add(
            ...
            y,
            0,
            f"Blu-ray Project Monitor | {project}",
            width - 1,
            color("accent_bold", curses.A_BOLD),
        )
        y += 1
        ...
        if not meta.get("manifest"):
            safe_add(
                ...
                y,
                0,
                "No media manifest yet. Initial analysis should have run; press r to retry display or w for full autopilot.",
                width - 1,
                color("warn_bold", curses.A_BOLD),
            )
            y += 1
            ...
            if workflow:
                safe_add(
                    ...
                    y,
                    0,
                    f"Autopilot: {workflow.get('status', '-')} step={workflow.get('step', '-')} pid={workflow.get('pid') or '-'} log={workflow.get('log_file', workflow_log_path(project))}",
                    width - 1,
                )
            stdscr.refresh()
            key = stdscr.getch()
            ...
            if key in (ord("w"), ord("W")):
                ...
                workflow = read_workflow_state(project)
                blockers = (
                    blocking_preflight_issues(project, root, cfg)
                    if ok
                    else [{"message": prompt_msg or "menu template generation failed"}]
                )
                if blockers:
                    message = "Preflight blocked autopilot: " + blockers[0].get(
                        "message", "fix project inputs first"
                    )
                elif (
                    workflow.get("pid")
                    and workflow.get("status") == "running"
        ...
        safe_add(
            ...
            0,
            "Tools: "
            + "  ".join(f"{k}: {'ok' if v else 'missing'}" for k, v in simple_tools.items()),
            width - 1,
            color("bad_bold" if missing else "ok"),
        )
        y += 1
        safe_add(
            stdscr,
            y,
            0,
            "GPU: " + (gpu or "NVIDIA/NVENC unavailable"),
            width - 1,
            color("ok" if gpu else "warn_bold"),
        )
        y += 1
        ...
        safe_add(
            ...
            y,
            0,
            f"Overall: {overall_text}  {meta.get('done_count', 0)}/{meta.get('total_count', 0)} done  {bar(30, overall)}",
            width - 1,
            color(
                "ok_bold" if meta.get("done_count") == meta.get("total_count") else "info_bold",
                curses.A_BOLD,
            ),
        )
        y += 1
        ...
        safe_add(
            ...
            y,
            0,
            f"Title: {cfg.get('disc_title') or project.name} | Options: disc={disc_label} menu={cfg.get('menu_backend', DEFAULT_MENU_BACKEND)} encoder={cfg['encoder']} resolution={cfg['resolution']} quality={cfg['quality']} nvenc_preset={cfg['nvenc_preset']} audio={cfg['audio_bitrate']} only={cfg.get('only') or 'all'} smoke={cfg.get('smoke_seconds') or 'off'}",
            width - 1,
            opt_attr,
        )
        y += 1
        ...
        if workflow:
            ...
            safe_add(
                ...
                y,
                0,
                f"Autopilot: {w_status} step={w_step} pid={w_pid} log={workflow.get('log_file', workflow_log_path(project))}",
                width - 1,
                status_attr(w_status),
            )
            y += 1
            safe_add(
                stdscr, y, 0, "Autopilot steps:", width - 1, color("accent_bold", curses.A_BOLD)
            )
            y += 1
            ...
            for step in workflow_step_rows(workflow, meta):
                ...
                safe_add(
                    ...
                    y,
                    2,
                    f"{step['status'][:7]:<7} {pct_text} {bar(step_bar_width, pct)} {step['label']}",
                    width - 3,
                    status_attr(step["status"]),
                )
                y += 1
            for line in tail_text(Path(workflow.get("log_file") or workflow_log_path(project)), 2):
                ...
                y += 1
        iso_text = (
            f"ISO: {meta.get('final_iso')} ({human_size(meta.get('final_iso_size'))})"
            if meta.get("final_iso_exists")
            else f"ISO: not ready ({meta.get('final_iso')})"
        )
        safe_add(
            stdscr,
            y,
            ...
            width - 1,
            color(
                "ok_bold" if meta.get("final_iso_exists") else "warn_bold",
                curses.A_BOLD if meta.get("final_iso_exists") else curses.A_NORMAL,
            ),
        )
        y += 1
        ...
        selected_label = next(
            (b.get("label") for b in burners if b.get("device") == selected),
            "no optical burner detected",
        )
        safe_add(
            stdscr,
            y,
            0,
            f"Burner: {selected} {selected_label}  (v cycle, b burn ISO)",
            width - 1,
            color("ok" if burners else "warn_bold"),
        )
        y += 1
        ...
        if burn:
            ...
            safe_add(
                ...
                y,
                0,
                f"Burn: {b_status} device={burn.get('device', '-')} pid={burn.get('pid') or '-'} {burn.get('message', '')} log={burn.get('log_file', burn_log_path(project))}",
                width - 1,
                status_attr(b_status),
            )
            y += 1
        ...
        header = "{:<16} {:<9} {:>7} {:>10} {:>10} {:>8} {}".format(
            "File", "Status", "Pct", "Encoded", "Duration", "Size", "Progress"
        )
        ...
        for row in rows:
            ...
            y += 1
            detail = "    enc={} fps={} speed={} bitrate={} output={}".format(
                row.get("encoder") or "-",
                row["fps"] or "-",
                row["speed"] or "-",
                row["bitrate"] or "-",
        ...
        for _ in range(10):
            ...
            if key in (ord("r"), ord("R")):
                ...
                if reason:
                    rc, log = run_initial_analyze(root, project)
                    message = (
                        f"Re-analysis complete; log {log}"
                        if rc == 0
                        else f"Re-analysis failed rc={rc}; log {log}"
                    )
                else:
            ...
            if key in (ord("b"), ord("B")):
                ...
                if (
                    burn.get("pid")
                    and burn.get("status") == "running"
                    and pid_running(burn.get("pid"))
                ):
                    message = (
                        f"Burn already running PID {burn.get('pid')}; log {burn.get('log_file')}"
                    )
            ...
            if key in (ord("m"), ord("M")):
                cfg["menu_backend"] = cycle(
                    cfg.get("menu_backend", DEFAULT_MENU_BACKEND), list(MENU_BACKENDS)
                )
                save_config(project, cfg)
                break
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Curses monitor for Blu-ray media preparation progress."
    )
    parser.add_argument("project_dir", help="path to the Blu-ray project directory")
    parser.add_argument(
        "--once", action="store_true", help="print a noninteractive status snapshot"
    )
    ...
    if args.once:
        ...
        print(
            json.dumps(
                {"project": str(project), "meta": meta, "videos": rows}, default=str, indent=2
            )
        )
        return 0
